...
import { useQuiniela } from "../../hooks/useQuiniela.ts";
import Loader from "../../components/ui/Loader.tsx";
import { QuinielaType } from "../../types/QinielaType.ts";

const Quinielas = () => {
    const { getQuinielaList, quinielaList } = useQuiniela()
    const { navigateTo } = useUser()
    const [loading, setLoading] = useState(true)

    const fetchQuinielas = async () => {
        try {
            await getQuinielaList()
            console.log("quinielaList", quinielaList);
    ...
    return (
        <main className="px-4">
            ...
            <table className="flex gap-4 flex-wrap">
                {loading ? <Loader /> :
                    quinielaList.map((item: QuinielaType, index) => (
                        <div key={index} className="grow basis-1/4 min-w-[500x]">
                            <div className="flex flex-col border rounded-lg border-gray-300 shadow-lg h-full">
                                ...
                                <div className="p-4 flex flex-col">
                                    <span className="text-xl font-semibold mb-4">{item.nombreQuiniela}</span>
                                    <div className="flex justify-around">
                                        <div className="text-gray-500 flex flex-col items-center gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-calendar" viewBox="0 0 16 16">
                                                <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4z" />
                                            </svg>
                                            <span className="text-gray-500 font-semibold">{item.eventos.length} partidos</span>
                                        </div>
                                        ...
                                        <div className=" flex flex-col items-center gap-2">
                                            ...
                                            <span className="text-gray-500 font-semibold">{item.premioAcumulado} $</span>
                                        </div>
                                ...
                                <div className="p-4">
                                    <Boton onClick={() => navigateTo(USER_ROUTES.QUINIELA.replace(':id', String(item.idQuiniela)))}>Armar quiniela</Boton>
                                </div>
                            </div>
                        </div>
                    ))
