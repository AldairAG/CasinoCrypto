...
const Boton: React.FC<BotonProps> = ({ children, className, ...props }) => {
  return (
    <button
      {...props}
      className={twMerge(`relative overflow-hidden px-6 py-3 bg-blue-700 text-white font-bold rounded-md transition-all duration-300 group`, className)}
    >
      {children}
